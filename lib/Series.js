...
module.exports = class Manga {
  ...
  browseSeries(series_type, {year, season, type, status, genres, genres_exclude, sort = 'popularity', order = 'desc', airing_data, full_page, page} = {}) {
    if (year && isNaN(year)) throw new Error(`${year} is not a valid value for year!`);
    ...
    if (series_type === 'anime') {
      if (status && this._statusTypesAnime[status]) {
        ...
        throw new Error(`${status} is not a valid value for status with series_type: '${series_type}'!`);
      }
      ...
      if (airing_data) throw new Error(`airing_data cannot be used for series_type: '${series_type}'!`);
    }

    if (sort && !this._sortFilters[sort]) throw new Error(`${sort} is not a valid value for sort!`);
    if (order && !this._orderTypes[order]) throw new Error(`${order} is not a valid value for order!`);
